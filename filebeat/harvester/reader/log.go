...
func (r *logFileReader) Read(buf []byte) (int, error) {
	...
	for {
		...
		logp.Debug("modification", "n = %d, err = %s", n, err)

		nilIdx := bytes.IndexByte(buf, 0)

		logp.Debug("modification", "nilIdx %d", nilIdx)
		...
		if nilIdx != -1 {
			...
			if seeker, ok := r.fs.(io.Seeker); ok {
				logp.Debug("seeker", "seeking to %d", r.offset+int64(n))

				_, err2 := seeker.Seek(r.offset+int64(n), os.SEEK_SET)

				if err2 != nil {
					logp.Err("Failed to seek for file %s; error: %s", r.fs.Name(), err2)
				}
			}
		}

		logp.Debug("modification2", "n = %d, err = %s", n, err)
